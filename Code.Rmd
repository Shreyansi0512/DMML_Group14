...
result.svm.ply.train, result.svm.radial.train) %>% 
...
```{r,  fig.pos = "H", out.width = '70%', fig.cap = "\\label{fig:tree} Decision Tree"}
...
table(data.train.scaled$y, pred.train.rf)
result.rf.train <- error_metrics(table(data.train.scaled$y, pred.train.rf))
...
For the selection of the best model, the most crucial rate to be considered in this case is the Sensitivity. Sensitivity gives the true positive rate i.e the probability of accurately predicting a 'yes'. In this case, the 
