...
data.train.char <- data.train %>%
  select(job, marital, education, housing, contact, poutcome)

data.train.num <- data.train.num %>%
  map_dfc(scale)

data.train.char <- data.train.char %>%
  map_dfc(as.factor)

data.train.scaled <- cbind(data.train %>% select(age, month, day_of_week, campaign, previous, n_contact, y), data.train.char, data.train.num)
