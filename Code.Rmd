...
data.train.char <- data.train %>%
  select(campaign, job, marital, education, housing, contact, poutcome)

means = sapply(data.train.num,mean)
sds = sapply(data.train.num, sd)

data.train.num = (data.train.num-means)/ sds


data.train.char <- data.train.char %>%
  map_dfc(as.factor)

data.train.scaled <- cbind(data.train %>% select(age, month, day_of_week, previous, n_contact, y), data.train.char, data.train.num)
...
data.train.num = (data.test.num-means)/ sds

data.test.char <- data.test.char %>%
  map_dfc(as.factor)

data.test.scaled <- cbind(data.test %>% select(age, month, day_of_week, previous, n_contact, y), data.test.char, data.test.num)
