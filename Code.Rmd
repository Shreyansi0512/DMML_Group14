...
accuracy = function(cm_table){
  cm_table = as.data.frame(cm_table)
  return((cm_table[1,'Freq'] + cm_table[1,'Freq'])/(cm_table[1,'Freq'] + cm_table[2,'Freq'] + cm_table[3,'Freq'] + cm_table[4,'Freq']))
}
...
error_metrics = function(cm_table){
  ...
  temp = data.frame(rbind(c(s,sp,p,a,f1)))
...
error_metrics(train.tab2)) %>% 
  ...
  kable(caption = '\\label{tab: KNN} 
        Results for K-nearest neighbours', digits=2) %>%
  kable_styling(font_size = 10, latex_options = "hold_position")
