...
The sensitivity for this model is `r result.gbm.train[1]`% and the accuracy is almost `r result.gbm.train[4]`%. Overall, this appears to be a good fit to the data. 
...
data.test.num <- data.test.scaled %>% dplyr::select(cons.conf.idx, cons.price.idx, emp.var.rate, euribor3m, nr.employed)
data.test.num.pca <- princomp(data.test.num, cor = T)
data.test.scaledlda <- data.test.scaled %>% 
  bind_cols(data.test.num.pca$scores[, 1:3]) %>% 
  select(-cons.conf.idx, cons.price.idx, emp.var.rate, euribor3m, nr.employed)
```
...
result.lda.test <- error_metrics(tab.gbm.test)
```
...
The sensitivity and precision for the optimal LDA model was estimated to be 0.21 and 0.66 respectively. A 10-fold cross-validation approach was undertaken to estimate the rates. 
