...
data.train.char <- data.train %>%
  select(campaign, job, marital, education, housing, contact, poutcome, n_contact)
...
data.train.scaled <- cbind(data.train %>% select(age, month, day_of_week, previous, y), data.train.char, data.train.num)
...
data.test.scaled <- cbind(data.test %>% select(age, month, day_of_week,previous, y), data.test.char, data.test.num)
